...
Property Get item() As String
    If pipe = 0 Then ErrRaise ERROR_INVALID_DATA
    BugAssert pBuffer <> Null
    Dim c As Long
    '取出BSTR的长度
    CopyMemory c, ByVal pBuffer, 4
    If c Then
        ' 由于未知原因，此处先将字符串放大一倍
        item = String$(c * 2, " ")
        CopyMemoryToStr item, ByVal (pBuffer + 4), c * 2
        Dim pos As Long
...
Property Let item(s As String)
    ...
    ' 重新将BSTR放回内存中
    CopyMemory ByVal pBuffer, c, 4
    CopyMemoryStr ByVal (pBuffer + 4), s, c * 2
End Property
'
